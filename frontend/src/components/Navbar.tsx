...
const NavItem: React.FC<NavItemProps> = ({ name, href, icon, isActive, onClick }) => {
    return (
        <a 
            href={href}
            onClick={onClick}
            className={`flex items-center md:gap-1 px-3 py-2 rounded-full text-sm font-medium transition-colors cursor-pointer
                ${isActive 
...
const navItemsData = [
    ...
    { name: 'Calendar', href: '#', icon: <CalendarIcon />, isActive: false },
...
const Navbar: React.FC<NavbarProps> = ( { onLogout, onChangePage } ) => {
    ...
    return (
        ...
        bg-white text-slate-500 z-50 rounded-t-2xl md:rounded-b-2xl shadow-lg px-2 md:px-8">

            <div className="flex justify-between items-center h-full">
                <div>
                    <h1 className="text-[18px] md:text-[24px] font-bold">TaskManager</h1>
                </div>
                ...
                {isProfileOpen && user && (
                    <div className={`absolute top-[-130px] md:top-22 right-4 w-60 p-2 space-y-2 bg-white border border-slate-200 
